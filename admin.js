...
function productForm(p={}){const vars=p.variants||[], imgs=p.images||[];return `<div class="drawer-head"><div><div class="small">${p.id?'EDITAR PRODUTO':'NOVO PRODUTO'}</div><h2>${p.id?esc(p.name):'Adicionar produto'}</h2></div><button class="modal-x" onclick="$('productDrawer').innerHTML='<div class=drawer-empty>Selecione um produto.</div>'">×</button></div><form class="form" onsubmit="saveProduct(event)"><input id="editId" type="hidden" value="${p.id||''}"><label>Nome<input id="n" required value="${esc(p.name||'')}"></label><div class="form-two"><label>Preço<input id="p" type="number" step=".01" value="${p.price??''}" required></label><label>Preço promocional<input id="pp" type="number" step=".01" value="${p.promotional_price??''}"></label></div><label>Categoria<select id="c">${categories.map(c=>`<option value="${c.id}" ${String(c.id)===String(p.category_id)?'selected':''}>${esc(c.name)}</option>`).join('')}</select></label><label>Descrição<textarea id="d" rows="6">${esc(p.description||'')}</textarea></label><div class="checks"><label><input id="active" type="checkbox" ${p.id?(p.active?'checked':''):'checked'}> Publicado</label><label><input id="featured" type="checkbox" ${p.featured?'checked':''}> Destaque</label></div><div class="subsection"><div class="sub-head"><h3>Fotos</h3><span>Quadrada recomendada: 1200 × 1200 px</span></div><div class="gallery-admin">${imgs.map(i=>`<div class="gallery-item"><img src="${esc(i.image_url)}"><button type="button" onclick="removeProductImage('${i.id}','${p.id}')">×</button></div>`).join('')}<label class="add-photo">+<small>Adicionar</small><input id="photos" type="file" accept="image/*" multiple></label></div></div><div class="subsection"><div class="sub-head"><h3>Variações, fotos e estoque</h3><button type="button" class="btn" onclick="addVariantRow()">+ Variação</button></div><p class="variant-photo-help">Escolha uma foto da galeria para cada variação. A mesma foto pode ser usada em vários tamanhos.</p><div id="variantRows">${vars.map(v=>variantRow(v,imgs)).join('')}</div></div><div class="drawer-actions"><button class="btn solid">Salvar produto</button>${p.id?`<button type="button" class="mini-danger text" onclick="deleteProduct('${p.id}')">Excluir produto</button>`:''}</div><div id="formMsg" class="form-msg"></div></form>`}
function normalizeVariantImages(imgs=[]){return imgs.map((item,i)=>({value:item.value??item.image_url??'',preview:item.preview??item.image_url??'',label:item.label||`Foto ${i+1}`})).filter(x=>x.value)}
function variantImageOptions(imgs=[],selected=''){return `<option value="">Sem foto específica</option>`+normalizeVariantImages(imgs).map(x=>`<option value="${esc(x.value)}" data-preview="${esc(x.preview)}" ${x.value===selected?'selected':''}>${esc(x.label)}</option>`).join('')}
function variantRow(v={},imgs=[]){const selected=v.image_url||'',preview=normalizeVariantImages(imgs).find(x=>x.value===selected)?.preview||selected;return `<div class="variant-row"><input class="v-color" placeholder="Cor" aria-label="Cor" value="${esc(v.color||'')}"><input class="v-size" placeholder="Tamanho" aria-label="Tamanho" value="${esc(v.size||'')}"><input class="v-sku" placeholder="SKU" aria-label="SKU" value="${esc(v.sku||'')}"><input class="v-stock" type="number" min="0" placeholder="Estoque" aria-label="Estoque" value="${v.stock??0}"><label class="variant-image-picker"><img class="v-image-preview ${preview?'':'empty'}" src="${esc(preview||'assets/hero.svg')}" alt=""><span><b>Foto da variação</b><select class="v-image" onchange="previewVariantImage(this)">${variantImageOptions(imgs,selected)}</select></span></label><button type="button" class="mini-danger" onclick="this.parentElement.remove()" aria-label="Remover variação">×</button></div>`}
function currentVariantImageChoices(){const id=$('editId')?.value,p=adminProducts.find(x=>String(x.id)===String(id));const stored=(p?.images||[]).map((x,i)=>({value:x.image_url,preview:x.image_url,label:`Foto ${i+1}`}));const pending=[...document.querySelectorAll('.gallery-item.pending-photo img')].map((img,i)=>({value:`__new_${i}`,preview:img.src,label:`Nova foto ${i+1}`}));return [...stored,...pending]}
function previewVariantImage(select){const img=select.closest('.variant-image-picker')?.querySelector('.v-image-preview'),url=select.selectedOptions[0]?.dataset.preview||'';if(img){img.src=url||'assets/hero.svg';img.classList.toggle('empty',!url)}}
function refreshVariantImageOptions(){const choices=currentVariantImageChoices();document.querySelectorAll('.v-image').forEach(select=>{const selected=select.value;select.innerHTML=variantImageOptions(choices,selected);if([...select.options].some(o=>o.value===selected))select.value=selected;previewVariantImage(select)})}
function addVariantRow(){$('variantRows').insertAdjacentHTML('beforeend',variantRow({},currentVariantImageChoices()))}
...
async function uploadProductPhoto(id,f,pos){const ext=(f.name.split('.').pop()||'jpg').replace(/[^a-z0-9]/gi,'');const path=`${id}/${Date.now()}-${pos}.${ext}`;const {error}=await supabaseClient.storage.from('product-images').upload(path,f,{contentType:f.type||undefined});if(error)throw error;const url=supabaseClient.storage.from('product-images').getPublicUrl(path).data.publicUrl;const r=await supabaseClient.from('product_images').insert({product_id:id,image_url:url,position:pos});if(r.error)throw r.error;return url}
async function saveProduct(e){e.preventDefault();$('formMsg').textContent='Salvando...';try{const id=$('editId').value,payload={name:$('n').value.trim(),description:$('d').value,category_id:$('c').value||null,price:Number($('p').value),promotional_price:$('pp').value?Number($('pp').value):null,active:$('active').checked,featured:$('featured').checked};let prod;if(id){const r=await supabaseClient.from('products').update(payload).eq('id',id).select().single();if(r.error)throw r.error;prod=r.data}else{payload.slug=slugify(payload.name)+'-'+Date.now();const r=await supabaseClient.from('products').insert(payload).select().single();if(r.error)throw r.error;prod=r.data}const current=adminProducts.find(x=>String(x.id)===String(prod.id)),files=[...($('photos')?.files||[])],startPos=Math.max(-1,...(current?.images||[]).map(x=>Number(x.position)||0))+1,uploadedUrls=[];for(let i=0;i<files.length;i++)uploadedUrls.push(await uploadProductPhoto(prod.id,files[i],startPos+i));const rows=[...document.querySelectorAll('.variant-row')].map(r=>{const chosen=r.querySelector('.v-image').value,image_url=chosen.startsWith('__new_')?uploadedUrls[Number(chosen.replace('__new_',''))]||null:chosen||null;return{product_id:prod.id,color:r.querySelector('.v-color').value||null,size:r.querySelector('.v-size').value||null,sku:r.querySelector('.v-sku').value||null,stock:Number(r.querySelector('.v-stock').value||0),image_url,active:true}});const del=await supabaseClient.from('product_variants').delete().eq('product_id',prod.id);if(del.error)throw del.error;if(rows.length){const rr=await supabaseClient.from('product_variants').insert(rows);if(rr.error)throw rr.error}$('formMsg').textContent='Produto salvo ♡';await loadAdminProducts();editProduct(prod.id)}catch(err){$('formMsg').textContent='Erro: '+err.message}}
async function removeProductImage(imgId,pid){if(!confirm('Remover esta foto do produto?'))return;const product=adminProducts.find(x=>String(x.id)===String(pid)),photo=product?.images?.find(x=>String(x.id)===String(imgId));if(photo?.image_url){const vr=await supabaseClient.from('product_variants').update({image_url:null}).eq('product_id',pid).eq('image_url',photo.image_url);if(vr.error)return alert(vr.error.message)}const {error}=await supabaseClient.from('product_images').delete().eq('id',imgId);if(error)return alert(error.message);await loadAdminProducts();editProduct(pid)}
...
document.addEventListener('change',e=>{if(e.target?.id==='photos')setTimeout(refreshVariantImageOptions,0)});
Object.assign(window,{login,logout,showTab,previewPage,closeEditModal,saveVisualEdit,saveIdentity,saveInfo,addInstagramRow,saveInstagram,newCategory,editCategory,deleteCategory,newProduct,editProduct,addVariantRow,previewVariantImage,saveProduct,removeProductImage,deleteProduct,saveGuide,renderAdminList});
