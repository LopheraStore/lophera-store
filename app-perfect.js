...
function productGallery(p){
  const pics=[...new Set((p.images?.length?p.images:[p.image]).filter(Boolean))];
  const slides=pics.map((url,i)=>`<div class="product-slide" role="group" aria-label="Foto ${i+1} de ${pics.length}"><img src="${esc(url)}" alt="${esc(p.name)} — foto ${i+1}" loading="${i?'lazy':'eager'}" onerror="this.onerror=null;this.src='assets/hero.svg'"></div>`).join('');
  if(pics.length<2)return`<div class="product-gallery single" data-gallery-index="0"><div class="product-slides">${slides}</div></div>`;
  const thumbs=pics.map((url,i)=>`<button class="product-thumb ${i===0?'active':''}" type="button" onclick="showProductImage(${i})" aria-label="Ver foto ${i+1}" aria-current="${i===0?'true':'false'}"><img src="${esc(url)}" alt="" loading="lazy" onerror="this.onerror=null;this.src='assets/hero.svg'"></button>`).join('');
  return`<div class="product-gallery" data-gallery-index="0"><div class="product-slides" id="productSlides" onscroll="syncProductGallery()">${slides}</div><button class="gallery-arrow gallery-prev" type="button" onclick="stepProductImage(-1)" aria-label="Foto anterior">‹</button><button class="gallery-arrow gallery-next" type="button" onclick="stepProductImage(1)" aria-label="Próxima foto">›</button><div class="gallery-counter"><span id="galleryCurrent">1</span> / ${pics.length}</div><div class="product-thumbs" aria-label="Miniaturas do produto">${thumbs}</div></div>`;
}
function updateGalleryState(index){
  const gallery=document.querySelector('.product-gallery'),slides=document.getElementById('productSlides');if(!gallery||!slides)return;
  const max=slides.children.length-1,indexSafe=Math.max(0,Math.min(Number(index)||0,max));gallery.dataset.galleryIndex=String(indexSafe);
  gallery.querySelectorAll('.product-thumb').forEach((thumb,i)=>{thumb.classList.toggle('active',i===indexSafe);thumb.setAttribute('aria-current',i===indexSafe?'true':'false')});
  const counter=document.getElementById('galleryCurrent');if(counter)counter.textContent=String(indexSafe+1);
  gallery.querySelectorAll('.product-thumb')[indexSafe]?.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'});
}
function showProductImage(index,behavior='smooth'){const slides=document.getElementById('productSlides');if(!slides)return;const max=slides.children.length-1,indexSafe=Math.max(0,Math.min(Number(index)||0,max));slides.scrollTo({left:slides.children[indexSafe].offsetLeft,behavior});updateGalleryState(indexSafe)}
function stepProductImage(direction){const gallery=document.querySelector('.product-gallery');if(!gallery)return;showProductImage(Number(gallery.dataset.galleryIndex||0)+direction)}
function syncProductGallery(){cancelAnimationFrame(gallerySyncFrame);gallerySyncFrame=requestAnimationFrame(()=>{const slides=document.getElementById('productSlides');if(!slides||!slides.clientWidth)return;updateGalleryState(Math.round(slides.scrollLeft/slides.clientWidth))})}
function currentProductImage(p){const gallery=document.querySelector('.product-gallery'),slides=document.getElementById('productSlides'),index=Number(gallery?.dataset?.galleryIndex||0);return slides?.children?.[index]?.querySelector('img')?.src||p.image}
async function openProduct(id){const p=products.find(x=>x.id===String(id));if(!p)return;const vr=await supabaseClient.from('product_variants').select('id,color,size,stock,sku').eq('product_id',id).eq('active',true).order('id');if(vr.error){alert('Erro ao carregar as opções deste produto: '+vr.error.message);return}p.variants=vr.data||[];p.colors=[...new Set(p.variants.map(v=>v.color).filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),'pt-BR'));const sizeOrder=['PP','P','M','G','GG','XG','XGG'];p.sizes=[...new Set(p.variants.map(v=>v.size).filter(Boolean))].sort((a,b)=>{const ia=sizeOrder.indexOf(String(a).toUpperCase()),ib=sizeOrder.indexOf(String(b).toUpperCase());return(ia<0?99:ia)-(ib<0?99:ib)||String(a).localeCompare(String(b),'pt-BR')});const modal=document.getElementById('modal');modal.classList.add('open');document.getElementById('modalBody').innerHTML=`<button class="modal-close" onclick="closeModal()">×</button>${productGallery(p)}<div class="modal-content"><div class="tag">${esc(p.category)}${p.is_customizable?' · PERSONALIZÁVEL':''}</div><h2>${esc(shortName(p.name))}</h2><div class="price" style="font-size:20px">${p.promotional_price?`<span class="old-price">${money(p.price)}</span>`:''}${money(p.promotional_price||p.price)}</div><p class="desc">${esc(p.description||'Uma peça Lophera pensada para unir estilo, conforto e personalidade.')}</p>${p.colors.length?`<label>Cor</label><div class="option-row" data-kind="color">${p.colors.map((x,i)=>`<button class="option ${i===0?'selected':''}" onclick="pick(this,'color','${p.id}')">${esc(x)}</button>`).join('')}</div>`:''}${p.sizes.length?`<label style="display:block;margin-top:14px">Tamanho</label><div class="option-row" data-kind="size">${p.sizes.map((x,i)=>`<button class="option ${i===0?'selected':''}" onclick="pick(this,'size','${p.id}')">${esc(x)}</button>`).join('')}</div>`:''}${customizationBlock(p)}<div id="stockState" class="tag" style="margin:14px 0"></div><div class="product-actions"><button class="btn solid" id="addBtn" onclick="addCart('${p.id}')">Adicionar ao carrinho</button><button class="btn" onclick="shareProductChat('${p.id}')">💬 Tirar dúvida</button><button class="btn favorite-modal-btn" id="favModalBtn" onclick="toggleFavorite(event,'${p.id}')">${favoriteIds.has(String(p.id))?'♥ Favoritado':'♡ Favoritar'}</button></div><div id="productReviews" class="product-reviews"><div class="reviews-loading">Carregando avaliações…</div></div></div>`;refreshVariant(p);loadProductReviews(p.id)}
...
function refreshVariant(p){
  ...
  if(p.is_customizable){
    const color=selected('color'),idx=/preto/i.test(color)?1:0;if(p.images?.[idx])showProductImage(idx);
  }
}
...
function shareProductChat(id){const p=products.find(x=>x.id===String(id));if(!p)return;const img=currentProductImage(p);const payload={product_id:Number(p.id),name:shortName(p.name),price:Number(p.promotional_price||p.price),image:img,url:'https://lopherastore.com.br/loja.html?produto='+encodeURIComponent(p.id),color:selected('color'),size:selected('size')};closeModal();if(typeof window.lopheraChatShareProduct==='function')window.lopheraChatShareProduct(payload);else alert('O chat ainda está carregando. Tente novamente em um instante ♡')}
function addCart(id){
  ...
  if(found)found.qty++;else cart.push({key,id,variant_id:v.id,name:p.name,color:v.color||'',size:v.size||'',price:Number(p.promotional_price||p.price),qty:1,image:currentProductImage(p),customization_path:artPath||null,customization_filename:artName||null,customization_note:artNote||null});
...
window.boot=boot;window.setFilter=setFilter;window.openProduct=openProduct;window.showProductImage=showProductImage;window.stepProductImage=stepProductImage;window.syncProductGallery=syncProductGallery;window.shareProductChat=shareProductChat;window.uploadCustomArt=uploadCustomArt;window.loadProductReviews=loadProductReviews;window.pick=pick;window.addCart=addCart;window.showCart=showCart;window.removeCart=removeCart;window.closeModal=closeModal;window.closeCart=closeCart;window.toggleFavorite=toggleFavorite;window.syncCartCloud=syncCartCloud;
